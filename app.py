...
def get_total_number_of_results(keyword, filters, max_retries=3):
...
def split_total_into_batches(total, batch_sizes=100):
    batches = [(i, i + batch_sizes) for i in range(0, total, batch_sizes)]
    # Adjust the last batch to not exceed the total
    if batches:
        batches[-1] = (batches[-1][0], total)
    return batches

def get_job_posting_ids(keyword, filters, batch, max_retries=3):
...
def extract_all_job_posting_ids(keyword, filters, batches):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(get_job_posting_ids, keyword, filters, batch) for batch in batches]
...
def extract_full_name_bio_and_linkedin_url(job_posting_id, max_retries=3):
    api_request_url = f"https://www.linkedin.com/voyager/api/graphql?variables=(cardSectionTypes:List(HIRING_TEAM_CARD),jobPostingUrn:urn%3Ali%3Afsd_jobPosting%3A{job_posting_id},includeSecondaryActionsV2:true)&queryId=voyagerJobsDashJobPostingDetailSections.192f7d390557a58ace4431207ed4999e"
    headers = {
    'accept': 'application/vnd.linkedin.normalized+json+2.1',
...
if st.button('Generera fil'):
    with st.spinner('Genererar fil, ett ögonblick'):
        if linkedin_job_url:
            ...
            search_query = keyword_search.group(1) if keyword_search else None
            print(f"\nNew search! Keyword: {search_query}")

            search_filters = extract_filters_from_url(linkedin_job_url)
            print(f"Filters: {search_filters}")
            print(f"Type of filters: {type(search_filters)}")

            start_time = time.time()
            # total_number_of_results = get_total_number_of_results(keyword)
            total_number_of_results = get_total_number_of_results(search_query, search_filters)
            ...
            results = main(search_query, search_filters, batches, staff_threshold, under_threshold_keywords, over_threshold_keywords, int(max_people_per_company))
            end_time = time.time()
            if results == 0:
